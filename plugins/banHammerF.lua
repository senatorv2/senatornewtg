...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "*خطا 404\nناتوان در پیدا کردن فردی بااین یوزر نیم!")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "*خطا\nشما نمیتوانید صاحبان گروه/ساپورت ها/ادمین ها/سودوها را حذف کنید")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "شما نمیتوانید صاحبان گروه/ساپورت ها/ادمین ها/سودوها را بن کنید")
			return
        end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '> ['..user_id..' ] ازتمامی گروه ها و سوپرگروه ها مسدود شد! (Globally banned)')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '> ['..user_id..' ] ازتمامی گروه هاو سوپرگروه ها آزاد شد! (Unglobally banned)')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ایدی' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == 'کیک می' and msg.to.type == "chat" then-- /kickme
  ...
  if matches[1]:lower() == 'بن' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "*خطا \nشما نمیتوانید صاحب گروه/ساپورت/ادمین/سودو را اخراج کنید"
        end
  ...
  if matches[1]:lower() == 'رفع بن' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
...
if matches[1]:lower() == 'کیک' then
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '> ['..user_id..' ] ازتمامی گروه ها و سوپرگروه ها مسدود شد! (گولبال بن شد)'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '> ['..user_id..' ] ازتمامی گروه ها و سوپر گروه ها آزاد شد! (ازگولبال بن ازاد شد)'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "لیست بن گوبال" then -- Global ban list
    return banall_list()
  end
end
